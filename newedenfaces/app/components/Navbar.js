...
class Navbar extends React.Component {
    ...
    componentDidMount() {
        ...
        $(document).ajaxState(() => {
            NavbarActions.updateAjaxAnimation('fadeIn');//bring in loading indicator when an AJAX request is fired
        });

        $(document).ajaxComplete(() => {
            setTimeout(() => {
                NavbarActions.updateAjaxAnimation('fadeOut');//remove loading indicator on completing AJAX query
            }, 750);
        });
    }
    ...
    handleSubmit(event) {//submit handler for search function on the navbar
